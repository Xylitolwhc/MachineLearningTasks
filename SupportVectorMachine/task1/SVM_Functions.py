...
def __main__():
    train_data, train_labels = loadData("./task1_linear.mat")
    plotData(X=train_data, y=train_labels)
    linear_model = svmTrain_SMO(X=train_data, y=train_labels, C=1.0)
    visualizeBoundaryLinear(X=train_data, y=train_labels, model=linear_model)

    train_data, train_labels = loadData("./task1_gaussian.mat")
    plotData(X=train_data, y=train_labels)
    sigma = 0.1
    gaussian_kernel = gaussianKernel(train_data, sigma)
    gaussian_model = svmTrain_SMO(X=train_data, y=train_labels, C=10, kernelFunction='gaussian',
                                  max_iter=100, K_matrix=gaussian_kernel)
